...
export const metadata: Metadata = {
  title: "Bestsellers Accessoires — Ziziwatches",
  description: "Découvrez nos accessoires et bijoux les plus populaires.",
};
...
export default async function BestsellersAccessoiresPage({ params, searchParams }: PageProps) {
    ...
    <>
      ...
      <main className="min-h-screen">
        ...
        <div className="relative h-72 sm:h-80 lg:h-[420px] flex items-center justify-center">
          <Image src="/bijouxwomen.png" alt="Bestsellers Accessoires" fill className="object-cover object-center" priority />
          ...
          <div className="relative z-10 text-center px-6 pt-16">
            ...
            <p className="text-white/40 text-sm tracking-[0.4em] mt-3 uppercase">
              {locale === "en" ? "Best-selling accessories" : "Nos accessoires les plus populaires"}
            </p>
          </div>
        </div>
        ...
        <div className="section-padding py-10">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            ...
            <div className="flex items-center gap-3">
              ...
              {(Object.keys(SORT_LABELS) as (keyof typeof SORT_LABELS)[]).map((s) => (
                <a
                  key={s}
                  href={`/${locale}/bestsellers/accessoires?sort=${s}${q ? `&q=${q}` : ""}`}
          ...
          {products.length === 0 ? (
            <BestsellerEmpty label="Aucun bestseller accessoire pour l'instant" />
          ) : (
