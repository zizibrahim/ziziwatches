...
export const metadata: Metadata = {
  title: "Bestsellers — Ziziwatches",
  description: "Découvrez nos pièces les plus populaires.",
};
...
export default async function BestsellersPage({ params, searchParams }: PageProps) {
  ...
  const where: Record<string, unknown> = { isBestseller: true, status: "ACTIVE" };
      ...
      <main className="min-h-screen">
        ...
        <div className="relative h-72 sm:h-80 lg:h-[420px] flex items-center justify-center">
          ...
          <div className="relative z-10 text-center px-6 pt-16">
            ...
            <p className="text-white/40 text-sm tracking-widest mt-3 uppercase">
              {locale === "en" ? "Our most popular pieces" : "Nos pièces les plus populaires"}
            </p>
          </div>
        </div>
        ...
        <SubCategoryCircles circles={[
          { label: "Tous",        href: `/${locale}/bestsellers`,                                      image: "/star.png" },
          { label: "Montres",     href: `/${locale}/shop?category=montres-homme&sort=priceDesc`,       image: "/menwatche.png" },
          { label: "Femme",       href: `/${locale}/shop?category=montres-femme&sort=priceDesc`,       image: "/womenwatche.png" },
          { label: "Bijoux",      href: `/${locale}/shop?category=accessoires&sort=priceDesc`,         image: "/bijouxwomen.png" },
          { label: "Pack",        href: `/${locale}/packs`,                                            image: "/pack.png" },
        ...
        <div className="section-padding py-10">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            ...
            <div className="flex items-center gap-3">
              ...
              {(Object.keys(SORT_LABELS) as (keyof typeof SORT_LABELS)[]).map((s) => (
                <a
                  key={s}
                  href={`/${locale}/bestsellers?sort=${s}${q ? `&q=${q}` : ""}`}
          ...
          {products.length === 0 ? (
            <BestsellerEmpty label="Aucun bestseller pour l'instant" />
          ) : (
