...
import BestsellerToggle from "@/components/admin/BestsellerToggle";
...
export default async function AdminBestsellersPage({ params, searchParams }: PageProps) {
  ...
  const products = await prisma.product.findMany({
    ...
    orderBy: [{ isBestseller: "desc" }, { nameFr: "asc" }],
  });

  const bestsellers = products.filter((p) => p.isBestseller);
  const rest        = products.filter((p) => !p.isBestseller);
    ...
    <div className="p-6 lg:p-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-foreground/40 text-sm mt-1">
          {bestsellers.length} produit{bestsellers.length !== 1 ? "s" : ""} sélectionné{bestsellers.length !== 1 ? "s" : ""}
      ...
      <div className="flex gap-1 flex-wrap mb-8 border-b border-border pb-0">
        {TABS.map((tab) => {
          ...
          const href = tab.slug
            ? `/${locale}/admin/bestsellers?cat=${tab.slug}`
            : `/${locale}/admin/bestsellers`;
          return (
            <a
              key={tab.label}
              href={href}
        ...
        <section className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-[10px] tracking-[0.3em] uppercase text-foreground/50">
              Bestsellers actifs — {bestsellers.length}
            </h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {bestsellers.map((p) => <ProductCard key={p.id} product={p} highlighted />)}
          </div>
        </section>
      )}
      ...
      {rest.length === 0 ? (
        <div className="border border-dashed border-border py-10 text-center">
          <p className="text-foreground/25 text-sm">Tous les produits de cette catégorie sont déjà des bestsellers.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
...
function ProductCard({ product, highlighted }: { product: Product; highlighted: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-3 p-3 border transition-colors ${
      highlighted ? "border-amber-500/30 bg-amber-500/5" : "border-border bg-surface"
    }`}>
      ...
      <p className="text-gold text-sm font-medium shrink-0">{formatPrice(product.price)}</p>
      <BestsellerToggle productId={product.id} isBestseller={product.isBestseller} />
    </div>
  );
}
